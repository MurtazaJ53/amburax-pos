...
| Outstanding lint | 24 errors, all `react-hooks/set-state-in-effect` |
| Surfaces | Two — Android counter app, admin web. See [platform-targets.md](platform-targets.md) |
...
| **iOS app** | Android is the overwhelming majority of this market segment |
...
| **Fixing the 24 `set-state-in-effect` warnings** | A real pattern smell, but none is a bug. Worth doing during a refactor, not as its own task |
